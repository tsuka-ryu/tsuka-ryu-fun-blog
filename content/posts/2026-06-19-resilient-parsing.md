...
1. `f2(x: i32,, z: i32)`の引数リストをパースするときに、次の`fn`キーワードなどが出てきてしまったということは、すでに次の関数のパースが始まってしまっていることになります。このままパースすると大半が壊れた気になってしまいます。そのため、いまの関数をパースするloopからは`break`で脱出します
2. 想定外の値があっても、Errorとして消費する
...
fatalが起きたとき、「Expressionを返す関数なのに値が作れない」ため困る。oxcは`Result`や`?`でのエラー伝播を使用せず、`Dummy::dummy()`を捨て値として返す。これでコードがシンプルになる。
...
というか、そもそもoxcでは`Resilient`という言葉は使われてないですね（笑）。Opusくんどうなっとんねん（？）
