---
title: oxcのTypeScriptパーサーを読む 第3回 TypeScriptはどこに住んでいるか
description: oxc のリポジトリを地図として広げて、TypeScript の構文がどのファイルのどこに住んでいるかを、拡張子による枝の切り替わりまで含めて確かめます。
date: 2026-09-26
tags: ["コンパイラ", "パーサー", "oxc", "TypeScript"]
---

第1回では型の降下のはしごを下から上まで歩き、第2回では `f<T>(x)` という一行を入口から
AST が出てくるまで通しでトレースしました。どちらも、細い道を一本ずつ歩いた回です。

今回は視点を引きます。歩いた2本の道を地図の上に置き直して、TypeScript という言語が
この巨大なリポジトリのどこに住んでいるのかを確かめる回です。コードの引用は少なめ、
ファイル名と行数が多めになります。

読んでいるのは rev `1aa5ec11ce` です。以下に出てくる行数と出現回数は、その時点の
チェックアウトで自分が `wc -l` と `grep` を走らせて数えた実測値です。
...
埋め込まれています。第2回で通しトレースしたのがこの世界でした。
...
変わる例で、第2回で追いかけた `<` の投機はこの行の `.ts` 側でだけ起きていたことになります。
